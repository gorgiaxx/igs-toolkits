...
static void test_super(int *start, size_t *length) {
	...
	if (super.flags & CRAMFS_FLAG_FSID_VERSION_2) {
		...
		if (*length < super.size) {
			...
			fprintf(stderr, "warning: file extends past end of filesystem, length:0x%08lx is larger than super.siz:0x%08lx\n", *length, super.size);
		}
	}
	else {
...
static void test_crc(int start)
{
	...
	if (crc != super.fsid.crc) {
		printf("crc error, crc:0x%08lx, super.fsid.crc:0x%08lx\n", crc, super.fsid.crc);
...
static struct cramfs_inode *read_super(void)
{
	...
	{
		die(FSCK_UNCORRECTED, 0, "bad root offset (0x%08lx), super.flags(0x%08lx), super.flags & CRAMFS_FLAG_SHIFTED_ROOT_OFFSET(0x%08lx)", offset, super.flags, super.flags & CRAMFS_FLAG_SHIFTED_ROOT_OFFSET);
	}
...
static void do_directory(char *path, struct cramfs_inode *i)
{
	...
	while (count > 0) {
		...
		if (newlen == 0) {
			die(FSCK_UNCORRECTED, 0, "filename length is zero.\n offset:0x%08lx, newlen:0x%08lx, pathlen:0x%08lx, newpath:%s", offset, newlen, pathlen, newpath);
		}
