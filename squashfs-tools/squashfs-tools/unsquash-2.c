...
int read_super_2(char *source, squashfs_operations **s_ops, void *s)
{
	squashfs_super_block_22_v2 *sBlk_2 = s;

	int res = read_fs_bytes(fd, SQUASHFS_START, sizeof(*sBlk_2), sBlk_2);

	if(res == FALSE)
		return res;

	swap = 0;
	if(sBlk_2->s_magic == SQUASHFS_MAGIC_SWAP) {
		squashfs_super_block_3 sblk;
		ERROR("Reading a different endian SQUASHFS filesystem on %s\n", source);
		SQUASHFS_SWAP_SUPER_BLOCK_3(&sblk, sBlk_2);
		memcpy(sBlk_2, &sblk, sizeof(squashfs_super_block_3));
		swap = 1;
	}

	if (sBlk_2->s_magic != SQUASHFS_MAGIC) {
		ERROR("read_super_2: invalid magic number: 0x%X\n", sBlk_2->s_magic);
		return -1;
	}

	if(sBlk_2->igs_info[0] == ' ' && sBlk_2->igs_info[1] == '<' && sBlk_2->igs_info[2] == 'I' && sBlk_2->igs_info[3] == 'n')
	{
		squashfs_super_block_22_v2 *sBlk_2_v2 = s;
		/*
		* Check it is a SQUASHFS superblock
		*/

		sBlk.s.s_magic = sBlk_2_v2->s_magic;
		sBlk.s.inodes = sBlk_2_v2->inodes;
		sBlk.s.mkfs_time = sBlk_2_v2->mkfs_time;
		sBlk.s.block_size = sBlk_2_v2->block_size;
		sBlk.s.fragments = sBlk_2_v2->fragments;
		sBlk.s.block_log = sBlk_2_v2->block_log;
		sBlk.s.flags = sBlk_2_v2->flags;
		sBlk.s.s_major = 2;
		sBlk.s.s_minor = 2;
		sBlk.s.root_inode = sBlk_2_v2->root_inode;
		sBlk.s.bytes_used = sBlk_2_v2->bytes_used;
		sBlk.s.inode_table_start = sBlk_2_v2->inode_table_start;
		sBlk.s.directory_table_start = sBlk_2_v2->directory_table_start;
		sBlk.s.fragment_table_start = sBlk_2_v2->fragment_table_start;
		sBlk.s.inode_table_start = sBlk_2_v2->inode_table_start;
		sBlk.no_uids = sBlk_2_v2->no_uids;
		sBlk.no_guids = sBlk_2_v2->no_guids;
		sBlk.uid_start = sBlk_2_v2->uid_start;
		sBlk.guid_start = sBlk_2_v2->guid_start;
		sBlk.s.xattr_id_table_start = SQUASHFS_INVALID_BLK;

		printf("IGS banner: %s\n", sBlk_2_v2->igs_info);
		printf("version: 0x%08X 0x%08X\n", sBlk_2_v2->igs_fs_version_major, sBlk_2_v2->igs_fs_version_minor);
	}
	else
	{
		squashfs_super_block_22_v1 *sBlk_2_v1 = s;
		/*
		* Check it is a SQUASHFS superblock
		*/

		sBlk.s.s_magic = sBlk_2_v1->s_magic;
		sBlk.s.inodes = sBlk_2_v1->inodes;
		sBlk.s.mkfs_time = sBlk_2_v1->mkfs_time;
		sBlk.s.block_size = sBlk_2_v1->block_size;
		sBlk.s.fragments = sBlk_2_v1->fragments;
		sBlk.s.block_log = sBlk_2_v1->block_log;
		sBlk.s.flags = sBlk_2_v1->flags;
		sBlk.s.s_major = 2;
		sBlk.s.s_minor = 1;
		sBlk.s.root_inode = sBlk_2_v1->root_inode;
		sBlk.s.bytes_used = sBlk_2_v1->bytes_used;
		sBlk.s.inode_table_start = sBlk_2_v1->inode_table_start;
		sBlk.s.directory_table_start = sBlk_2_v1->directory_table_start;
		sBlk.s.fragment_table_start = sBlk_2_v1->fragment_table_start;
		sBlk.s.inode_table_start = sBlk_2_v1->inode_table_start;
		sBlk.no_uids = sBlk_2_v1->no_uids;
		sBlk.no_guids = sBlk_2_v1->no_guids;
		sBlk.uid_start = sBlk_2_v1->uid_start;
		sBlk.guid_start = sBlk_2_v1->guid_start;
		sBlk.s.xattr_id_table_start = SQUASHFS_INVALID_BLK;
	}
